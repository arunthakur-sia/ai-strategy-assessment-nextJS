...
function ts() { return `${DIM}${new Date().toISOString()}${RESET}` }
...
export const log = {
  projectCreate(name: string, entityName: string, projectId: string) {
    ...
    kv('Project ID', projectId); kv('Name', name); kv('Entity', entityName)
  ...
  collectionRequest(name: string, description: string, folderId: string) {
    ...
    kv('name', name, YELLOW); kv('description', description, YELLOW); kv('folderId', folderId, YELLOW)
    console.log(`  ${ts()}`); console.log()
  },
  collectionResponse(collectionId: string | null, name: string) {
    if (collectionId) {
      divider(GREEN, `[siagpt] ← 200  Collection Created`)
      kv('collectionId (uuid)', collectionId, GREEN); kv('name', name, GREEN)
    } else {
  ...
  docUploadRequest(fileName: string, mimetype: string, collectionId: string) {
    ...
    kv('file', fileName, YELLOW); kv('mimetype', mimetype, YELLOW); kv('collectionId', collectionId, YELLOW)
    console.log(`  ${ts()}`); console.log()
  },
  docUploadResponse(fileName: string, ok: boolean, status?: number) {
    if (ok) {
      divider(GREEN, `[siagpt] ← 200  Document Indexed`); kv('file', fileName, GREEN)
    } else {
      divider(RED, `[siagpt] ← ${status ?? '???'}  Document Upload Failed`); kv('file', fileName, RED)
    }
    console.log(`  ${ts()}`); console.log()
  },
  discussionRequest(name: string, ownerId: string) {
    divider(CYAN, `[siagpt] → POST /chat/discussions`)
    kv('name', name, CYAN); kv('ownerId', ownerId, CYAN); kv('ownerType', 'USER', CYAN)
    console.log(`  ${ts()}`); console.log()
  },
  ...
  messageRequest(payload: {
    question: string; messageId: string; discussionId: string; ownerId: string
    messageMetadata: Record<string, unknown>; context?: string
  }) {
    ...
    if ((payload.messageMetadata as any).assistantId) kv('assistantId', (payload.messageMetadata as any).assistantId, MAGENTA)
  ...
  messageResponse(raw: string, chosenEvent: string, context?: string) {
    ...
    const preview = raw.length > 1500 ? raw.slice(0, 1500) + `\n  ${DIM}... [+${raw.length - 1500} chars truncated]${RESET}` : raw
    preview.split('\n').forEach(l => console.log(`  ${l}`))
    console.log(`  ${ts()}`); console.log()
  },
  messageError(err: string, context?: string) {
    divider(RED, `[siagpt] ✗ Error  (${context ?? 'LLM Call'})`); kv('error', err, RED)
    console.log(`  ${ts()}`); console.log()
  },
  info(msg: string) { console.log(`${BLUE}${BOLD}[SIA]${RESET} ${ts()} ${msg}`) },
  warn(msg: string) { console.log(`${YELLOW}${BOLD}[WARN]${RESET} ${ts()} ${msg}`) },
  error(msg: string, err?: unknown) { console.error(`${RED}${BOLD}[ERROR]${RESET} ${ts()} ${msg}`, err ?? '') },
}
