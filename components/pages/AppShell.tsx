...
const NAV_ITEMS = [
  ...
  { path: '/initiatives', icon: Target, label: 'Initiatives', key: 'initiatives' },
  { path: '/outputs', icon: Download, label: 'Export Center', key: 'outputs' },
]
  ...
  return (
      ...
      <aside style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: '6px', cursor: 'pointer', textAlign: 'left', width: '100%', transition: 'all 0.15s',
                }}
              >
              ...
              {entities.map(entity => {
                ...
                return (
                  ...
                  >
                    <Building2 size={11} color={active ? '#A78BFA' : 'var(--sia-medium-gray)'} />
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ fontSize: '11px', fontWeight: active ? 700 : 400, color: active ? '#A78BFA' : 'var(--sia-medium-gray)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {entity.name}
                      </div>
                    ...
                    {score
                      ? <span style={{ fontSize: '11px', fontWeight: 700, color: active ? '#A78BFA' : 'var(--sia-medium-gray)', flexShrink: 0 }}>{score}</span>
                      : active && <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#A78BFA', flexShrink: 0 }} />
                    }
                  </button>
                )
              })}
        ...
        <nav style={{ flex: 1, padding: '12px 0', overflowY: 'auto' }}>
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button key={item.key} onClick={() => router.push(item.path)} data-testid={`nav-${item.key}`}
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
              {Object.entries(pillars).map(([id, p]: [string, any]) => {
                ...
                return (
                  <button key={id} onClick={() => router.push('/assessment')}
        ...
        <div style={{ padding: '12px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: '6px', flexDirection: sidebarCollapsed ? 'column' : 'row', alignItems: 'center' }}>
          <button onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')}
            title="Toggle language" data-testid="button-language"
    ...
    {project && (
      <button
        ...
        style={{
          position: 'fixed', right: sourcesOpen ? '300px' : '0px', top: '50%', transform: 'translateY(-50%)',
          zIndex: 50, background: 'var(--sia-navy)', color: '#fff', border: 'none',
          borderRadius: '6px 0 0 6px', padding: '12px 6px', cursor: 'pointer',
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px',
          boxShadow: '-2px 0 8px rgba(0,0,0,0.12)', transition: 'right 0.25s',
        }}
      >
        <BookOpen size={13} />
    ...
    {project && (
      <div style={{
        ...
        width: sourcesOpen ? '300px' : '0px', overflow: 'hidden', transition: 'width 0.25s',
        background: '#fff', borderLeft: '1px solid rgba(69,85,105,0.1)', zIndex: 45,
        display: 'flex', flexDirection: 'column', boxShadow: sourcesOpen ? '-4px 0 16px rgba(0,0,0,0.1)' : 'none',
      }}>
        {sourcesOpen && (
